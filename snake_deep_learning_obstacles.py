...
class SnakeEnv:
    ...
    def get_state(self):
        ...
        if obstacle is not None:
            obstacle_x_rel_snake_x = (self.snake_x - obstacle[0]) / screen_width
            obstacle_y_rel_snake_y = (self.snake_y - obstacle[1]) / screen_height
        else:
            obstacle_x_rel_snake_x = 0
            obstacle_y_rel_snake_y = 0

        state = np.array([snake_x_rel_food_x, snake_y_rel_food_y, food_left, food_right, food_up, food_down, direction_x, direction_y, obstacle_x_rel_snake_x, obstacle_y_rel_snake_y, int(obstacle is not None)])
    ...
    def get_closest_obstacle(self):
        ...
        for obstacle in self.obstacle_list:
            distance = np.sqrt((self.snake_x - obstacle[0]) ** 2 + (self.snake_y - obstacle[1]) ** 2)
    ...
    def add_obstacle(self):
        ...
        obstacle_x = random.randint(0, (screen_width - obstacle_width) // self.snake_speed) * self.snake_speed
        obstacle_y = random.randint(0, (screen_height - obstacle_height) // self.snake_speed) * self.snake_speed

        self.obstacle_list.append(pygame.Rect(obstacle_x, obstacle_y, obstacle_width, obstacle_height))
    ...
    def step(self, action):
        ...
        # Add obstacles if the score is divisible by 50
        if self.score % 50 == 0 and len(self.obstacle_list) < 3:
            self.add_obstacle()

            # Check for collisions with the obstacles
        for obstacle in self.obstacle_list:
            if self.snake_x < obstacle.x + obstacle.width and \
                    self.snake_x + self.snake_width > obstacle.x and \
                    self.snake_y < obstacle.y + obstacle.height and \
                    self.snake_y + self.snake_height > obstacle.y:
                done = True
                reward = -1
